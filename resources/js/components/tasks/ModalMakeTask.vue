        ...
        <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full"
                ...
                <div class="w-full sm:flex sm:items-start ">
                    <div class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left ">
                        ...
                        <input class="mt-4 px-2 py-2 border border-2 border-gray-300 rounded-lg w-full" type="text"
                               placeholder="Task description" v-model="name"/>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                    <a @click.prevent="makeOrUpdateTask"
...
<script>
    export default {
        ...
        methods: {
            ...
            async makeOrUpdateTask() {

                if (this.name.length > 0) {
                    if (this.id === -1) {
                        let response = await axios.post(`/api/projects/${this.project.id}/tasks`, {
                            name: this.name
                        });
                        this.$emit('addTask', response.data);
                    }else{
                        let response = await axios.patch(`/api/projects/${this.project.id}/tasks/${this.id}`, {
                            name: this.name
                        });
                        this.$emit('addTask', response.data);
                    }
